# Code source: https://github.com/cognitedata/Steady-State-Detector/blob/master/steady_state.py
...
def detect_ssd_cognite(feature_df, feature_name, window, make_plot: bool = True):
    model = SSDetector()
    #model = SSDetector(slope_threashold = 0.002, state_transition_probability = 0.5, number_particles = 200, n_timeless = 20)
    model.train(feature_df.values, show_results=True)

    feature_df['y_hat'] = model.y_hat
    feature_df['y_hat'] = feature_df.y_hat.shift(-1)
    feature_df['p'] = model.p_steady
    feature_df['p'] = feature_df.p.shift(-1)
    feature_df['is_steady'] = 0
    steady_idx = feature_df[feature_df['p'] > 0.98].index
    feature_df.loc[steady_idx, 'is_steady'] = 1
    if make_plot:
        make_ssd_plot_cognite(feature_df, feature_name, window)

    return feature_df

def make_ssd_plot_cognite(feature_df, feature_name, window):
    plt.rcParams["figure.figsize"] = (20, 10)
    ...
    ax = feature_df.case_arrivals.plot(color='black')
    ax.pcolorfast(ax.get_xlim(), ax.get_ylim(),
                  feature_df['is_steady'].values[np.newaxis], cmap=cmap,
                  alpha=0.9)
    ...
    plt.title(f"Steady State Detection (cognite): {feature_name}, {window}", fontsize='x-large')
    plt.savefig(f'fig_ssd_cognite_{feature_name}_{window}')
    plt.show()
