...
# Jianguo Wu, Yong Chen, “Online detection of steady-state operation using a multiple-change-point model and exact bayesian inference,” IIE Transactions, vol. 48, no. 7, pp. 599–613, 2016.
...
class ExactBayes:
    ...
    def _update(self):
        ...
        while self.t < self.size:
            ...
            if self.t > self.m - 1:
                locations = sorted(np.random.choice(list(range(self.t)), size=self.m - 1, replace=False,
                                                    p=self.P_lcp / np.sum(self.P_lcp)))  # m-1 locations from 1...t-1
            else:
            ...
            for j in locations:
                P_lcp_t[j] = (1. / (np.pi ** (1. / 2))) \
                             * np.power(
                    (np.linalg.det(self._M_st(j, self.t)) / np.linalg.det(self._M_st(j, self.t - 1))), 1. / 2) \
                             * np.power((self._H_st(j, self.t - 1) / self._H_st(j, self.t)),
                                        (self.t - j + self.nu - 1.) / 2) \
            ...
            for j in norm_P_lcp_t.keys():
                P_t += (norm.cdf((self.s_0 - self._muy_st(j, self.t)[0]) / (self._K_st(j, self.t)[0, 0] * (
                            (self._d_st(j, self.t) / (self._d_st(j, self.t) - 2)) ** (1. / 2)))) \
                        - norm.cdf((-self.s_0 - self._muy_st(j, self.t)[0]) / (self._K_st(j, self.t)[0, 0] * (
                                    (self._d_st(j, self.t) / (self._d_st(j, self.t) - 2)) ** (1. / 2))))) \
                       * norm_P_lcp_t[j]
    ...
    def _H_st(self, s, t):
        ...
        y = np.array([self.y[s:t + 1]]).T
        N_st = self._N_st(s, t)
        return np.dot(y.T, y) + self.gamma + np.dot(np.dot(self.beta_0.T, self.Sigma.I), self.beta_0) - np.dot(
            np.dot(N_st.T, self._M_st(s, t)), N_st)
...
def conduct_ssd_eb(signal, batch_size_min: int = 10, m: int = 20, s_0: float = 0.002):
