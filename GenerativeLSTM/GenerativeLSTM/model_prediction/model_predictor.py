...
# import readers.log_reader as lr  # replaced: broken PyPI package
...
class ModelPredictor():
    ...
    def execute_predictive_task(self):
        ...
        for run_num in range(0, self.parms['rep']):
            ...
            if self.parms['activity'] == 'pred_log':
                ...
                evaluator.evaluate(self.parms, self.log, self.predictions, run_num)
    ...
    # pandas' default na_values list includes the literal string "NA" -- real
    # case ids in sepsis.xes happen to collide with it (case "NA", 24 events),
    # so an unguarded read_csv silently corrupts that case's id to NaN and
    # drops it from the whole prediction loop. Every other default NA string
    # is kept so blank-cell handling elsewhere in this file is unaffected.
    _NA_VALUES_NO_NA = {
    ...
    def load_log_test(output_route, parms):
        df_test = pd.read_csv(
            ...
            keep_default_na=False, na_values=ModelPredictor._NA_VALUES_NO_NA,
            dtype={'caseid': str},
        )
        df_test['end_timestamp'] = pd.to_datetime(df_test['end_timestamp'], utc=True, format='ISO8601')
        if 'start_timestamp' in df_test.columns:
            df_test['start_timestamp'] = pd.to_datetime(df_test['start_timestamp'], utc=True, format='ISO8601')
...
class EvaluateTask():
    ...
    def _evaluate_predict_next(self, parms, log, predictions, rep_num):
        exp_desc = self.clean_parameters(parms.copy())
        evaluator = ev.Evaluator(parms['one_timestamp'])
        ac_sim = evaluator.measure('accuracy', predictions, 'ac')
        rl_sim = evaluator.measure('accuracy', predictions, 'rl')
        ...
        if parms['one_timestamp']:
            tm_mae = evaluator.measure('mae_next', predictions, 'tm')
            tm_mae = pd.concat([tm_mae, exp_desc], axis=1).to_dict('records')
            self.save_results(tm_mae, 'tm', parms)
        else:
            dur_mae = evaluator.measure('mae_next', predictions, 'dur')
            wait_mae = evaluator.measure('mae_next', predictions, 'wait')
    ...
    def _evaluate_pred_sfx(self, parms, log, predictions, rep_num):
        exp_desc = self.clean_parameters(parms.copy())
        evaluator = ev.Evaluator(parms['one_timestamp'])
        ac_sim = evaluator.measure('similarity', predictions, 'ac')
        rl_sim = evaluator.measure('similarity', predictions, 'rl')
        mean_sim = ac_sim['mean'].mean() if 'mean' in ac_sim.columns else float('nan')
        if len(ac_sim) == 0:
            return mean_sim
        exp_desc = pd.DataFrame([exp_desc])
        exp_desc = pd.concat([exp_desc]*len(ac_sim), ignore_index=True)
        ...
        self.save_results(rl_sim, 'rl', parms)
        if parms['one_timestamp']:
            tm_mae = evaluator.measure('mae_suffix', predictions, 'tm')
            tm_mae = pd.concat([tm_mae, exp_desc], axis=1).to_dict('records')
            self.save_results(tm_mae, 'tm', parms)
        else:
            dur_mae = evaluator.measure('mae_suffix', predictions, 'dur')
            wait_mae = evaluator.measure('mae_suffix', predictions, 'wait')
            dur_mae = pd.concat([dur_mae, exp_desc], axis=1).to_dict('records')
            wait_mae = pd.concat([wait_mae, exp_desc], axis=1).to_dict('records')
            self.save_results(dur_mae, 'dur', parms)
            self.save_results(wait_mae, 'wait', parms)
