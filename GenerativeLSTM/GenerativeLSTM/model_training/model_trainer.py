...
# import readers.log_splitter as ls  # replaced: broken PyPI package
...
class ModelTrainer():
    ...
    @staticmethod
    def load_log(params):
        log_df = pd.read_csv(os.path.join('input_files', params['file_name']))
        log_df['end_timestamp'] = pd.to_datetime(log_df['end_timestamp'], utc=True)
        if 'start_timestamp' in log_df.columns:
            log_df['start_timestamp'] = pd.to_datetime(log_df['start_timestamp'], utc=True)
        for col in ['Unnamed: 0', 'role']:
    ...
    def split_timeline(self, size: float, one_ts: bool) -> None:
        ...
        Preferred method: timeline_contained — only complete traces in each partition.
        Falls back to timeline_trace (sorted by case start) if test < 10% of log.
        """
        key = 'end_timestamp' if one_ts else 'start_timestamp'
        log = self.log.sort_values(key).reset_index(drop=True)
        split_point = log[key].quantile(size)

        # timeline_contained: only traces fully before / fully after the split point
        case_max = log.groupby('caseid')[key].max()
        case_min = log.groupby('caseid')[key].min()
        train_cases = case_max[case_max <= split_point].index
        test_cases  = case_min[case_min  > split_point].index
        train = log[log['caseid'].isin(train_cases)]
        test  = log[log['caseid'].isin(test_cases)]

        # timeline_trace fallback: sort cases by start time, take first `size` fraction
        if len(test) < int(len(log) * 0.1):
            case_start  = log.groupby('caseid')[key].min().sort_values()
            n_train     = int(len(case_start) * size)
            train_cases = case_start.index[:n_train]
            test_cases  = case_start.index[n_train:]
            train = log[log['caseid'].isin(train_cases)]
            test  = log[log['caseid'].isin(test_cases)]

        self.log_test  = test.sort_values(key).reset_index(drop=True)
        self.log_train = train.sort_values(key).reset_index(drop=True)
