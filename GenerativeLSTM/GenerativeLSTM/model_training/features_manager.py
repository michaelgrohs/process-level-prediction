...
class FeaturesMannager():
    ...
    def add_calculated_times(self, log):
        ...
        log = sorted(log, key=lambda x: str(x['caseid']))
        for _, group in itertools.groupby(log, key=lambda x: x['caseid']):
            ...
            for i in range(0, len(events)):
                ...
                events[i]['weekday'] = events[i][ordk].weekday()
    ...
    def scale_feature(log, feature, method, replace=False):
        ...
        if method == 'lognorm':
            ...
            log[feature+'_norm'] = (np.divide(
                    np.subtract(log[feature+'_log'], min_value), denom)
                    if denom > 0 else 0)
            ...
            denom = max_value - min_value
            log[feature+'_norm'] = (np.divide(
                    np.subtract(log[feature], min_value), denom)
                    if denom > 0 else 0)
            scale_args = {'max_value': max_value, 'min_value': min_value}
        elif method == 'standard':
            mean = np.mean(log[feature])
            std = np.std(log[feature])
            log[feature + '_norm'] = (np.divide(np.subtract(log[feature], mean), std)
                                      if std > 0 else 0)
