...
class SlurmWorker():
    ...
    def load_log_test(output_route, parms):
        df_train = pd.read_csv(os.path.join(output_route, 'train.csv'))
        df_train['end_timestamp'] = pd.to_datetime(df_train['end_timestamp'], utc=True)
        if 'start_timestamp' in df_train.columns:
            df_train['start_timestamp'] = pd.to_datetime(df_train['start_timestamp'], utc=True)
    ...
    @staticmethod
    def split_timeline(size: float, log: pd.DataFrame, one_ts: bool):
        """
        ...
        Falls back to timeline_trace (sorted by case start) if valdn < 10% of log.
        """
        key = 'end_timestamp' if one_ts else 'start_timestamp'
        log = log.sort_values(key).reset_index(drop=True)
        ...
        valdn_cases = case_min[case_min  > split_point].index
        train = log[log['caseid'].isin(train_cases)]
        valdn = log[log['caseid'].isin(valdn_cases)]

        if len(valdn) < int(len(log) * 0.1):
            case_start  = log.groupby('caseid')[key].min().sort_values()
            n_train     = int(len(case_start) * size)
            train_cases = case_start.index[:n_train]
            valdn_cases = case_start.index[n_train:]
            train = log[log['caseid'].isin(train_cases)]
            valdn = log[log['caseid'].isin(valdn_cases)]

        log_valdn = valdn.sort_values(key).reset_index(drop=True)
        log_train = train.sort_values(key).reset_index(drop=True)
