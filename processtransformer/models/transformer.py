# -*- coding: utf-8 -*-
"""
Created on Thu Jan 16 07:03:30 2025
This script is based on the following source code:
    https://github.com/Zaharah/processtransformer
We just adjusted some parts to efficiently use it in our experiments.
"""
...
def get_next_activity_model(max_case_length, vocab_size, output_dim, 
    ...
    x = transformer_block(x, training=True)  # Pass training=True explicitly
    # TODO: remove the following comment (original code resolve in debugging)
...
def get_next_role_model(max_case_length, vocab_size, output_dim, 
    embed_dim = 36, num_heads = 4, ff_dim = 64):
    inputs = layers.Input(shape=(max_case_length,))
    x = TokenAndPositionEmbedding(max_case_length, vocab_size, embed_dim)(inputs)
    transformer_block = TransformerBlock(embed_dim, num_heads, ff_dim)
    ...
    outputs = layers.Dense(output_dim, activation="linear")(x)
    transformer = tf.keras.Model(inputs=inputs, outputs=outputs,
        name = "next_role_transformer")
    return transformer


def get_next_time_model(max_case_length, vocab_size, output_dim = 2, 
    embed_dim = 36, num_heads = 4, ff_dim = 64):

    inputs = layers.Input(shape=(max_case_length,))
    # Three time-based features
    time_inputs = layers.Input(shape=(5,)) 
    x = TokenAndPositionEmbedding(max_case_length, vocab_size, embed_dim)(inputs)
    transformer_block = TransformerBlock(embed_dim, num_heads, ff_dim)
    x = transformer_block(x, training=True)  # Pass training=True explicitly
    # TODO: remove the following comment (original code resolve in debugging)
