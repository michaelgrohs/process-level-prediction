...
=======================
Standalone prediction script for Bukhsh and Camargo models trained on
real-life event logs via run_experiments_real.py.

Discovers trained models under:
  best_models/{dataset}/{trim}/bukhsh/best_params.json
  best_models/{dataset}/{trim}/camargo/hpo_summary.json

For each (dataset, trim) pair found, generates predictions for:
  - Full-trace test set   (FULL_TRACES=True, all completed test cases)
  - Half-prefix test set  (ceil(N/2) events per case)

For Bukhsh two sets of KPI metrics are computed per run:
  metrics_{run_name}.csv      — CC/TT from the predicted suffix event log
  metrics_{run_name}_rt.csv   — CC/TT from the remaining-time output
                                 (2-event log: case start + predicted end)

For Camargo one set of metrics is computed:
  metrics_{run_name}.csv      — CC/TT from the predicted suffix event log

Raw prediction artefacts saved by the trainers:
  Bukhsh full  →  best_models/{ds}/{trim}/bukhsh/{run_name}/event_log.csv
                                                             /rem_time.csv
  Bukhsh half  →  best_models/{ds}/{trim}/bukhsh/{run_name}/half_prefix/event_log.csv
                                                                         /rem_time.csv
  Camargo      →  GenerativeLSTM/GenerativeLSTM/output_files/{trim}/{run_name}/gen_*.csv
                  (+ event_log.csv written by this script to the camargo results dir)

KPI results written to:
  results/bukhsh_hpo/{trim}/{run_name}/
  results/bukhsh_hpo_half/{trim}/{run_name}/
  results/camargo_hpo/{trim}/{run_name}/
  results/camargo_hpo_half/{trim}/{run_name}/

Each results folder contains:
  metrics_{run_name}.csv      — CC/TT MSE + MAE (suffix)
  metrics_{run_name}_rt.csv   — CC/TT MSE + MAE (RT, Bukhsh only)
  time_{run_name}.csv         — wall-clock prediction time
  concurrent_cases.png        — actual vs predicted CC series
  throughput_time.png         — actual vs predicted TT series

Usage
-----
  # Activate your venv first, then from the project root:
  ...
  python run_predictions_real.py --dataset bpic20-dom --trim none --model bukhsh

Skip logic
----------
  A (dataset, trim, model, mode) combination is skipped if the primary
  metrics CSV already exists in the corresponding results folder. Delete
  the CSV to force a re-run.
"""
